...
struct shared_slab_allocator {
	...
	host_version->extra_memory = host_ptr_ext_mem;
 	...
 	host_version->malloc_containers = per_size_pinned_blocks::generate_on_device(extra_blocks);
 	...
 	boot_memory_kernel<my_type><<<(extra_blocks-1)/512+1, extra_blocks>>>(dev_version, extra_blocks);
 	cudaDeviceSynchronize();
	...
	__device__ void boot_block(int smid){
		...
		Block * new_block = (Block *) block_allocator->get_mem_from_offset(slab_offset);
