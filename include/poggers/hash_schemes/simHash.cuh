...
struct __attribute__ ((__packed__)) static_size_iceberg_table {
	...
	__device__ bool insert_key_pair(cg::thread_block_tile<Partition_size> group, key_val_pair<Key, Val> pair){
		...
		if (!insert_pair_into_primary_bucket(group, pair, generate_slot_primary_bucket(primary_hash, secondary_hash))){
	...
	__device__ void insert_pairs_device_side(key_val_pair<Key, Val> * pairs,  uint64_t nkeys){
	...
	__device__ void insert_pairs_device_side_misses(key_val_pair<Key, Val> * pairs, uint64_t nkeys, uint64_t * misses){
	...
	__device__ void query_keys_device_side(Key * keys,  uint64_t nkeys){
