#ifndef DISCRETE_BUCKET_INSERT 
#define DISCRETE_BUCKET_INSERT
...
public:
	...
	using my_type = bucket_insert<Key, Val, Partition_Size, Internal_Rep, Max_Probes, Hasher, Probing_Scheme>;
	...
	__host__ static my_type * generate_on_device(uint64_t ext_nslots, uint64_t ext_seed){
		...
		uint64_t min_buckets = (ext_nslots-1)/Partition_Size+1;

		uint64_t true_nslots = min_buckets*Partition_Size;
	...
	__device__ __inline__ bool insert(cg::thread_block_tile<Partition_Size> insert_tile, Key key, Val val){
		...
		for (uint64_t insert_slot = insert_probing_scheme.begin(key); insert_slot != insert_probing_scheme.end(); insert_slot = insert_probing_scheme.next()){
       		...
       		insert_slot = insert_slot*Partition_Size + insert_tile.thread_rank();
