...
struct templated_bitbuddy_four {
  ...
  __device__ uint64_t malloc_group(int num_contiguous) {
    while (true) {
      int index =
          mask.shrink_index(mask.get_first_x_contiguous(num_contiguous));
    ...
    while (true) {
      ...
      if (index != -1) {
        ...
        if (offset != (~0ULL)) {
          ...
          // if we failed to read from one of these, then it must not be
          // available
  ...
  // 		int index =
  // mask.shrink_index(mask.get_random_active_bit_control());
...
// 	using bitbuddy_type =
// templated_bitbuddy_four<determine_depth<num_allocations>::depth,
// determine_num_allocations<determine_depth<num_allocations>::depth>::count>;
...
// 		host_version.internal_allocator =
// bitbuddy_type::generate_on_device();
...
// 		cudaMalloc((void **)&ext_memory,
// num_allocations*size_of_allocation);

// 		if (ext_memory == nullptr){

// 			cudaFree(host_version.internal_allocator);

// 			printf("Allocator could not get enough memory to handle
...
// 		//my type is 16 bytes. I'm gonna conservatively estimate that this
// will always go through. 		cudaMalloc((void **)&dev_version, sizeof(my_type));
...
// 		uint64_t offset =
// internal_allocator->malloc_offset((bytes_needed-1)/size_of_allocation+1);
...
// 		uint64_t offset = ((uint64_t) allocation - (uint64_t)
// memory)/size_of_allocation;
...
// 		//all allocations from the bitbuddy must be in the stride of the
// bitbuddy.
// 		//since internals are offset by light of their allocation this is
// an easy check. 		return ((uint64_t) allocation % bitbuddy_type::lowest_size ==
// allocation);
