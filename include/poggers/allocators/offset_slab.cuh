...
// does modify
__device__ int select_unique_bit_int(cg::coalesced_group &active_threads,
...
struct warp_lock {
  ...
  __device__ void init() { lock_bits = 0ULL; }

  __device__ int get_warp_bit() { return (threadIdx.x / 32); }

  __device__ bool lock() { return lock_bits.set_bit_atomic(get_warp_bit()); }

  __device__ void unlock() { lock_bits.unset_bit_atomic(get_warp_bit()); }
...
struct offset_alloc_bitarr {
  ...
  __device__ void attach_allocation(uint64_t ext_offset) {
    ...
    if ((internal_offset % 4096) != 0) {
      printf(
          "Logical bug in bucket reasoning! bits up to 4096 not actually "
          "free\n");
    }
  }
  ...
  // 	uint64_t scanned_mask = cg::inclusive_scan(starting_team, my_mask,
  // cg::bit_or<uint64_t>());

  // 	if (starting_team.thread_rank() == starting_team.size()-1){

  // 		uint64_t old_bits =
  // alloc_bits[upper_bit].set_OR_mask(scanned_mask);
  ...
  // 			if (manager_bits.set_index(upper_bit) &
  // SET_BIT_MASK(upper_bit)){
  ...
  __device__ bool bit_malloc_v2(cg::coalesced_group &active_threads,
                                uint64_t &offset, uint64_t &remainder) {
    // if (active_threads.thread_rank() == 0){
    ...
    if (!check_indices(active_threads, my_index)) {
      printf(
          "Team %d with %d threads, Bug in select unique main alloc index %d\n",
...
struct offset_storage_bitmap {
  ...
  __device__ bool bit_malloc_v3(cg::coalesced_group &active_threads,
                                uint64_t &offset, uint64_t &remainder) {
#if SLAB_GLOBAL_LOADING
    manager_bits.global_load_this();
#endif
  ...
  // 	#if DEBUG_PRINTS
  // 	if (active_threads.thread_rank() == 0){
  // 		printf("%d/%d %llx\n", active_threads.thread_rank(),
  // active_threads.size(), local_copy);
  // 	}
  // 	#endif
  ...
  // 			//allocation_index_bit =
  // local_copy.get_first_active_bit();

  // 			allocation_index_bit =
  // local_copy.get_random_active_bit();

  // 		}

  // 		allocation_index_bit = active_threads.shfl(allocation_index_bit,
  // 0);
  ...
  // 			if
  // (manager_bits.unset_bit_atomic(allocation_index_bit)){

  // 				ext_bits =
  // alloc_bits[allocation_index_bit].swap_to_empty();
  ...
  // active_threads.thread_rank(), active_threads.size(), allocation_index_bit,
  // ext_bits);
  // 			}
  // 			#endif

  // 			if (active_threads.thread_rank()+1 <=
  // ext_bits.get_fill()){
  ...
  // 				//options
  // 				//1) grab an allocation of the first n and try
  // to
  // 				//2) select the first n bits ahead of time.

  // 				//int bits_needed =  (ext_bits.get_fill() -
  // active_threads.size());

  // 				//int my_bits =
  // bits_before_index(active_threads.thread_rank());

  // 				// bool ballot = (bits_needeed == my_bits);

  // 				// int result =
  // coalesced_threads.ballot(ballot);

  // 				int my_index;

  // 				while (true){

  // 					cg::coalesced_group searching_group =
  // cg::coalesced_threads();

  // 					my_index =
  // ext_bits.get_random_active_bit();
  ...
  // 					//any threads still searching group
  // together
  // 					//do an exclusive scan on the OR bits

  // 					//if the exclusive OR result doesn't contain your bit you
  // are free to modify!

  // 					//last thread knows the true state of the system, so
  // broadcast changes.

  // 					uint64_t my_mask = (1ULL) << my_index;

  // 					//now scan across the masks
  // 					uint64_t scanned_mask = cg::exclusive_scan(searching_group,
  // my_mask, cg::bit_or<uint64_t>());

  // 					//final thread needs to broadcast
  // updates 					if (searching_group.thread_rank() == searching_group.size()-1){

  // 						//doesn't matter as the scan only adds
  // bits
  // 						//not to set the mask to all bits not
  // taken 						uint64_t final_mask = ~(scanned_mask | my_mask);

  // 						ext_bits.apply_mask(final_mask);

  // 					}

  // 					//everyone now has an updated final copy of ext
  // bits? 					ext_bits = searching_group.shfl(ext_bits, searching_group.size()-1);

  // 					if (!(scanned_mask & my_mask)){

  // 						//I received an item!
  // 						//allocation has already been marked and index is
  // set
  // 						//break to recoalesce for exit
  // 						break;
  ...
  // 				allocation = (void *) (memmap[allocation_index_bit] +
  // my_index);

  // 				//someone now has the minimum.
  // 				int my_fill = ext_bits.get_fill();

  // 				int lowest_fill = cg::reduce(coalesced_threads, my_fill,
  // cg::less<int>());

  // 				int leader = __ffs(coalesced_threads.ballot(lowest_fill ==
  // my_fill))-1;

  // 				#if DEBUG_PRINTS
  // 				if (leader == coalesced_threads.thread_rank()){
  // 					printf("Leader reports lowest fill: %d, my_fill: %d, bits:
  // %llx\n", lowest_fill, my_fill, ext_bits);
  // 				}
  // 				#endif
  // 				//printf("Leader is %d\n", leader,
  // coalesced_threads.size());

  // 				if ((ext_bits.get_fill() > 0) && (leader ==
  // coalesced_threads.thread_rank())){
...
__device__ bool alloc_with_locks(uint64_t &allocation,
                                 offset_alloc_bitarr *manager,
                                 offset_storage_bitmap *block_storage) {
  ...
  // if 100% of requests are satisfied, we are all returning, so one thread
  // needs to drop lock.
  ...
  bool bit_malloc_result =
      manager->bit_malloc_v2(remaining, allocation, remainder);
  ...
  if (bit_malloc_result) {
    ...
    // offset_alloc_bitarr * alt_bitarr = (offset_alloc_bitarr *)
    // block_allocator->get_mem_from_offset(debug_alloc_bitarr_offset);
  ...
  if (remaining.thread_rank() == 0) {
    ...
    if (__popcll(remainder) > 0 && bit_malloc_result) {
      bool result = block_storage->attach_buffer(allocation - (allocation % 64),
                                                 remainder);
...
__global__ void init_storage(offset_storage_bitmap *storages,
                             int num_storages) {
...
struct pinned_storage {
  ...
  static __host__ pinned_storage *generate_on_device(int device) {
    ...
    // sizeof(offset_storage_bitmap)*num_storages);
    cudaMalloc((void **)&dev_storages,
               sizeof(offset_storage_bitmap) * num_storages);

    init_storage<<<(num_storages - 1) / 256 + 1, 256>>>(dev_storages,
                                                        num_storages);
    ...
    cudaMemcpy(dev_ptr, host_storage, sizeof(pinned_storage),
  ...
  static __host__ void free_on_device(pinned_storage *dev_storage) {
    ...
    cudaMemcpy(host_storage, dev_storage, sizeof(pinned_storage),
...
// __device__ bool alloc_with_locks_no_storage(uint64_t & allocation,
// offset_alloc_bitarr * manager, offset_storage_bitmap * block_storage){
...
// 	//this can't partially fail - I think.
// 	//should only ever return teams of 64 or total bust
// 	bool bit_malloc_result = manager->bit_malloc_v2(remaining, allocation,
// remainder);
...
// 		  //only attempt to attach if not empty.
// 	      if (__popcll(remainder) > 0){
// 		      //bool result = block_storage->attach_buffer(allocation -
// (allocation % 64), remainder);
...
// 				bitmaps[index].init();
// 				bitmaps[index].attach_allocation(memory +
// 4096*alloc_size*index);
...
// 		int index = ((uint64_t) bitmap - (uint64_t )
// bitmaps)/sizeof(alloc_bitarr);
...
struct smid_pinned_storage {
  ...
  // __device__ bool attach_new_buffer(int index, offset_alloc_bitarr *
  // new_buffer){
  ...
  __device__ bool attach_new_buffer(int index,
                                    offset_alloc_bitarr *new_buffer) {
  ...
  __device__ void init_with_allocators_memory(block_allocator *balloc,
    ...
    for (int i = 0; i < num_backups + 1; i++) {
      ...
      offset_alloc_bitarr *slab =
          (offset_alloc_bitarr *)balloc->get_mem_from_offset(slab_offset);
...
__global__ void smid_pinned_block_init_storage(
    ...
    smid_pinned_storage<num_blocks> *storages, int num_storages) {
  uint64_t tid = threadIdx.x + blockIdx.x * blockDim.x;
...
struct smid_pinned_container {
  ...
  static __host__ my_type *generate_on_device(int device,
    ...
    smid_pinned_block_init_storage<num_blocks, block_allocator,
                                   memory_allocator>
        <<<(num_storages - 1) / 256 + 1, 256>>>(balloc, memalloc, dev_storages,
                                                num_storages);
