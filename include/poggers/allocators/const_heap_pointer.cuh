 ...
 public:
  ...
  __device__ static my_type *init_superblock(void *superblock_space) {
    ...
    for (uint64_t i = 0; i < pointers_per_superblock; i++) {
      ...
      modified_ptr[i].distance_and_counter =
          my_type::merge_counter_and_pointer(0, 1);

#if DEBUG_ASSERTS

      assert(my_type::get_offset_from_mixed(
                 modified_ptr[i].distance_and_counter) == 1);
      assert(my_type::get_counter_from_mixed(
                 modified_ptr[i].distance_and_counter) == 0);
    ...
    assert(
        my_type::get_offset_from_mixed(
            modified_ptr[pointers_per_superblock - 1].distance_and_counter) ==
        0);
    assert(
        my_type::get_counter_from_mixed(
            modified_ptr[pointers_per_superblock - 1].distance_and_counter) ==
        0);
...
struct bytes_given_wrapper {
  using heap_ptr =
      const_heap_pointer<bytes_per_item, bytes_given / bytes_per_item>;
};
...
struct manager {
  ...
  using pointer_type =
      typename bytes_given_wrapper<bytes_per_item,
                                   bytes_given - sizeof(my_type)>::heap_ptr;
  ...
  __device__ void *malloc() { return get_stack()->malloc(); }
