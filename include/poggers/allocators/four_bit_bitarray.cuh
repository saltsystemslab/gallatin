...
struct four_bit_bitarray {
  ...
  __host__ __device__ four_bit_bitarray() { bits = 0ULL; }

  __host__ __device__ four_bit_bitarray(uint64_t ext_bits) { bits = ext_bits; }
  ...
  __device__ inline uint64_t unset_lock_bit_atomic(int index) {
    return atomicAnd((unsigned long long int *)this,
                     UNSET_LOCK_BIT_FOUR(index));
  }
  ...
  __device__ inline uint64_t unset_alloc_bit_atomic(int index) {
    return atomicAnd((unsigned long long int *)this,
                     UNSET_ALLOC_BIT_FOUR(index));
  }

  __device__ inline uint64_t set_continue_bit_atomic(int index) {
    return atomicOr((unsigned long long int *)this,
                    SET_CONTINUE_BIT_FOUR(index));
  }

  __device__ inline uint64_t unset_continue_bit_atomic(int index) {
    return atomicAnd((unsigned long long int *)this,
                     UNSET_CONTINUE_BIT_FOUR(index));
  }
