...
__global__ void stack_init_kernel(uint64_t superblock_as_uint, int bytes_in_use,
                                  int bytes_per_item) {
...
struct aligned_heap_ptr {
 ...
 public:
  ...
  __host__ __device__ static inline uint get_offset_from_mixed(
      unioned_bitfield ext_dist_and_counter) {
    return ext_dist_and_counter.as_bitfield.offset;
  }

  __device__ static inline uint get_counter_from_mixed(
      unioned_bitfield ext_dist_and_counter) {
    return ext_dist_and_counter.as_bitfield.counter;
  }

  __device__ static unioned_bitfield merge_counter_and_pointer(uint counter,
                                                               uint offset) {
  ...
  __device__ static void init_stack_helper(uint64_t superblock_as_uint,
                                           int bytes_in_use,
                                           int bytes_per_item) {
    uint64_t tid = threadIdx.x + blockIdx.x * blockDim.x;

    my_type *modified_ptr =
        (my_type *)(superblock_as_uint + bytes_in_use + tid * bytes_per_item);

    my_type *next_modified_ptr = (my_type *)(superblock_as_uint + bytes_in_use +
                                             (tid + 1) * bytes_per_item);
    ...
    if (next_modified_ptr->get_home_address() == superblock_as_uint) {
      uint64_t next_offset =
          modified_ptr->get_offset_from_next(next_modified_ptr);
      modified_ptr->distance_and_counter =
          my_type::merge_counter_and_pointer(0, next_offset);

    } else {
      modified_ptr->distance_and_counter =
          my_type::merge_counter_and_pointer(0, 0);
    }

    return;
  }

  __device__ static my_type *init_stack_team(void *superblock_space,
                                             int bytes_in_use,
                                             int bytes_per_item) {
#if COUNTING_CYCLES
    ...
    stack_init_kernel<my_type><<<(max_calls - 1) / 512 + 1, 512>>>(
        superblock_as_uint, bytes_in_use, bytes_per_item);
    ...
    uint64_t aligned_stack_init_time =
        (stack_init_counter_end - stack_init_counter_start) / COMPRESS_VALUE;

    atomicAdd((unsigned long long int *)&stack_init_counter,
              (unsigned long long int)aligned_stack_init_time);

    atomicAdd((unsigned long long int *)&stack_init_traversals,
  ...
  __device__ static my_type *init_stack(void *superblock_space,
                                        int bytes_in_use, int bytes_per_item) {
    // printf("Booting stack\n");

    my_type *my_stack =
        init_stack_team(superblock_space, bytes_in_use, bytes_per_item);

    // my_type * my_stack = init_stack_old(superblock_space, bytes_in_use,
    // bytes_per_item);
  ...
  // request for a thread to allocate the entirety of a superblock to this
  // sizing necessary for HOARD implementation might come back later with a
  // parallel implementation for coop groups
  __device__ static my_type *init_stack_old(void *superblock_space,
                                            int bytes_in_use,
                                            int bytes_per_item) {
#if COUNTING_CYCLES
    ...
    while (true) {
      my_type *next_modified_ptr =
          (my_type *)(superblock_as_uint + bytes_in_use +
                      (i + 1) * bytes_per_item);
    ...
    // modified_ptr[pointers_per_superblock -1].distance_and_counter =
    // my_type::merge_counter_and_pointer(0, 0);

    // assert(my_type::get_offset_from_mixed(modified_ptr[pointers_per_superblock
    // -1].distance_and_counter) == 0);
    // assert(my_type::get_counter_from_mixed(modified_ptr[pointers_per_superblock
    // -1].distance_and_counter) == 0);
  ...
  __device__ unioned_bitfield atomicload() {
    ...
    ret_bitfield.as_uint =
        (atomicCAS((unsigned int *)&distance_and_counter, 0U, 0U));
    return ret_bitfield;
  }
  ...
  __device__ inline uint get_offset_from_next(my_type *next) {
    ...
    // shifts always record themselves as shift/4, as the minimum size allowed
    // is 4 bytes (anyone else is secretly promoted!) this allows us to store 4x
    // more items per block then before while still

    return shift >> 2;
  }

  // atomic to update the state based on what we saw the last time we did an
  // atomic read
  __device__ inline bool atomic_set_value(unioned_bitfield offset,
                                          unioned_bitfield replacement) {
    if (atomicCAS((unsigned int *)&distance_and_counter,
                  (unsigned int)offset.as_uint,
                  (unsigned int)replacement.as_uint) ==
        (unsigned int)offset.as_uint) {
  ...
  __device__ inline bool set_next(unioned_bitfield old_counter,
                                  uint new_counter, my_type *other) {
    uint replacement_next = my_type::get_offset_from_next(other);

    unioned_bitfield merged =
        my_type::merge_counter_and_pointer(new_counter, replacement_next);
  ...
  __device__ inline void *malloc() {
    ...
    while (true) {
      ...
      uint next_offset =
          my_type::get_offset_from_mixed(next_node->atomicload());
  ...
  __device__ inline void free(void *allocation) {
    ...
    while (true) {
      ...
      uint casted_new_counter =
          my_type::get_counter_from_mixed(casted_merged) + 1;

      if (casted_allocation->set_next(casted_merged, casted_new_counter,
                                      head_next)) {
...
//     using heap_ptr = aligned_heap_ptr<bytes_per_item,
//     bytes_given/bytes_per_item>;
// };

// //template metaprogramming to find the next side
// template <std::size_t bytes_per_item>
// struct first_matching_address{
// 	static const uint64_t value = std::max(bytes_per_item, (std::size_t)
// 16);
// };
...
struct aligned_manager {
  ...
  __device__ static my_type *init_stack(void *ext_address, int bytes_per_item) {
    ...
    pointer_type *head = pointer_type::init_stack(
        ext_address,
        max((unsigned long)bytes_per_item, (unsigned long)sizeof(my_type)),
        bytes_per_item);

    // init_stack(void * superblock_space, int bytes_in_use, int
    // bytes_per_item){
  ...
  __device__ static my_type *init_from_free_list(header *free_list,
                                                 int bytes_per_item) {
    void *address_to_use =
        free_list->malloc_aligned(bytes_given, bytes_given, 0);
  ...
  __device__ void *malloc() {
    ...
    uint64_t aligned_stack_total_time =
        (stack_counter_end - stack_counter_start) / COMPRESS_VALUE;

    atomicAdd((unsigned long long int *)&stack_counter,
              (unsigned long long int)aligned_stack_total_time);

    atomicAdd((unsigned long long int *)&stack_traversals,
              (unsigned long long int)1);
  ...
  __device__ my_type *get_next() { return ptr_to_next; }

  __device__ my_type *get_next_atomic() {
    return (my_type *)atomicCAS((unsigned long long int *)&ptr_to_next, 0ULL,
                                0ULL);
  }

  __device__ my_type *get_prev_atomic() {
    return (my_type *)atomicCAS((unsigned long long int *)&ptr_to_prev, 0ULL,
                                0ULL);
  }
  ...
  __device__ void set_next_atomic(my_type *next) {
    atomicExch((unsigned long long int *)&ptr_to_next,
               (unsigned long long int)next);
  }

  __device__ void set_prev_atomic(my_type *prev) {
    atomicExch((unsigned long long int *)&ptr_to_prev,
  ...
  // this is useful for grabbing the head node when we know we must eventually
  // succeed
  ...
  __device__ void unlock() { free_lock(); }
  ...
  // raw bytes available
  __device__ int total_bytes(int alignment_size) { return bytes_given; }

  // raw bytes used
  __device__ int bytes_used(int alignment_size) {
    return total_bytes(alignment_size) -
           count_available(alignment_size) * alignment_size;
  }
