...
struct mixed_counter {
  ...
  bool assert_valid() { return (first == second); }
};
...
struct atomicLock {
  ...
  __device__ inline ~atomicLock() { header_to_lock->unlock(); }
};
...
struct header {
  ...
  __device__ bool lock() { return lock_and_size.lock(); }
  ...
  __device__ void unlock() { return lock_and_size.unlock(); }
  ...
  __device__ bool replace(header_bitfield old, header_bitfield new_status) {
    return (atomicCAS((unsigned long long int *)(&lock_and_size), old.as_uint,
                      new_status.as_uint) == old.as_uint);
  }

  __device__ bool check_alloc() { return atomic_get() & ALLOCED_MASK; }

  __device__ bool check_lock() { return atomic_get() & LOCK_MASK; }

  __device__ bool check_endpoint() { return atomic_get() & ENDPOINT_MASK; }
  ...
  __device__ uint64_t get_size() { return lock_and_size.as_bitfield.size; }

  __device__ void set_next(header *next_node) { next_ptr = next_node; }

  __device__ void set_prev(header *prev_node) { prev_ptr = prev_node; }

  __host__ __device__ header *get_next() { return next_ptr; }

  __host__ __device__ header *get_prev() { return prev_ptr; }
  ...
  __device__ void printnode() {
    uint64_t tid = threadIdx.x + blockIdx.x * blockDim.x;
    printf(
        "%llu: Node at %p, size is %llu, next at %p, prev at %p, is head: %d, "
        "is_locked: %d, is alloced: %d\n",
        tid, this, lock_and_size.as_bitfield.size, get_next(), get_prev(),
        check_endpoint(), check_lock(), check_alloc());
  }
  ...
  __device__ void static init_heap_global(void *unsigned_allocation,
                                          uint64_t num_bytes) {
    ...
    printf("%llu, %llu, %llu\n", heap_start->get_size(), main_node->get_size(),
           heap_end->get_size());
  ...
  // dequeue a node
  __device__ static void *allocate_next(header *prev, header *main,
                                        header *next) {
    prev->set_next(next);
    next->set_prev(prev);
...
// 	//on in use-segments lock is not required as they can only be modified
// by a free
...
// 	__device__ bool replace_bitfield(header_bitfield old, header_bitfield
// replacement){

// 		if (atomicCAS((unsigned long long int *)&lock_and_size,
// old.as_uint, replacement.as_uint) == old.as_uint){ 			return true;
// 		}
...
// 	__device__ inline void unlock_and_replace(uint64_t old_size, uint64_t
// new_size){
...
// 		char * correct_address = ((char *) this) + 16 +
// lock.as_bitfield.size;
...
// 		heap_wrapper.header->lock_and_size.as_bitfield.size =
// bytes_inside; 		heap_wrapper.header->lock_and_size.lock = 0;

// 		heap_wrapper.footer = (memory_ptr *) (address + num_bytes - 16);

// 		heap_wrapper.footer->lock_and_size.as_bitfield.size =
// bytes_inside; 		heap_wrapper.footer->lock_and_size.lock = 0;
...
// 		main_heap_node = heap_wrapper::create_new_node(address+32,
// num_bytes-64);

// 		dummy_heap_end =
// heap_wrapper::create_new_node(address+num_bytes-32, 32);
...
// 			ret_field.as_uint = atomicCAS((unsigned long long int
// *)&counters, 0, 0); 			return ret_field;
...
// 		__device__ bool atomic_swap(large_unioned_bitfield current,
// large_unioned_bitfield replace){

// 			if (current.as_uint == atomicCAS((unsigned long long int
// *)&counters, current.as_uint, replace.as_uint)){
...
// 		__device__ uint64_t offset_to_next_aligned_region(uint64_t
// alignment_width){

// 			uint64_t this_as_bits = (uint64_t) this;

// 			//this as bits - (this_as bits % aligment) is how far off
// alignment we are
// 			// aligment - ^quantity is how many bytes to the next correct
// region

// 			uint64_t offset = alignment_width - (this_as_bits -
// (this_as_bits % alignment_width));
...
// 		__device__ large_unioned_bitfield
// lock_node_in_exclusive_state(){

// 			large_unioned_bitfield status =
// get_node_in_valid_state(); 			large_unioned_bitfield new_status = status;
...
// 		__device__ large_unioned_bitfield
// lock_node_in_exclusive_state(large_unioned_bitfield old_status){
...
// 		__device__ void unlock_node_from_status(large_unioned_bitfield
// locked_status){
...
// 			//this should always succeed as no one can generate a new
// valid state from
// 			// our locked state
// 			#if DEBUG_ASSERTS
...
// 		__device__ void * request_allocation_from_block(uint64_t
// num_bytes){

// 			//first check just to assert node is valid
// 			large_unioned_bitfield status =
// get_node_in_valid_state();
...
// 			large_unioned_bitfield locked_status =
// lock_node_in_exclusive_state(status);
...
// 			//otherwise we can request! figure out how many bytes to
// detach 			uint64_t new_width = bytes_available
...
// 		__device__ void * request_aligned_allocation(uint64_t num_bytes,
// uint64_t alignment_width){

// 		}

// 		__host__ global_heap_ptr * generate_on_device(uint64_t
// num_bytes){
...
// 			cudaMemcpy(dev_heap, &host_heap, sizeof(global_heap),
...
// 			cudaMemcpy(&host_heap, dev_heap, sizeof(global_heap),
