...
struct sub_allocator {
  ...
  __device__ static my_type *init(header *heap) {
    ...
    for (int i = 0; i < maximum_p2 - 2; i++) {
      ...
      // new_allocator->managers[i] = stack_type::init_from_free_list(heap, 1ULL
      // << (2+i));
    ...
    while (true) {
      ...
      while (local_manager != nullptr) {
        ...
        if (malloced == nullptr) {
          ...
          //  this code is just in case the ptr main had to the end was updated
          //  by someone else likely impossible but you never know ¯\_(ツ)_/¯
          {
          ...
          // this will create a loop that only this thread will be able to
          // unwind
          ...
          //  this will push all full nodes out of the list so that only this
          //  thread has a ptr to them.
          // now anyone currently traversing will filter out of the loop

          // A->C->B->C->B

          // consider atomic swap here
          // atomicCAS((unsigned_long long int *) managers + p2_needed,
...
struct sub_allocator_wrapper {
  using sub_allocator_type =
      sub_allocator<bytes_per_substack, log2_template<max_size - 1>::value + 1>;
};
