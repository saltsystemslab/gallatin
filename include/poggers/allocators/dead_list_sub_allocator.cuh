...
// be assigned to trawl the dead list and look for stacks that have had active
// frees. This should amortize out and allow new mallocs to occur in roughly
// constant time.
...
struct dead_list_sub_allocator {
  ...
  __device__ static my_type *init(header *heap) {
    my_type *new_allocator =
        (my_type *)heap->malloc_aligned(sizeof(my_type), 16, 0);
    ...
    printf("Booting manager with %llu stacks, expecting %llu bytes used\n",
           maximum_p2 - 2,
           sizeof(my_type) + bytes_per_substack * (maximum_p2 - 2));
#endif
    ...
    for (int i = 0; i < maximum_p2 - 2; i++) {
      ...
      new_allocator->managers[i] =
          stack_type::init_from_free_list(heap, 1ULL << (2 + i));
  ...
  // While this doesn't require that the allocator belongs to the heap it is
  // being freed to you should return it to the same free list
  __device__ static void free_allocator(header *heap,
                                        my_type *allocator_to_free) {
    for (int i = 0; i < maximum_p2 - 2; i++) {
  ...
  __device__ bool swap_manager_atomic(int p2, stack_type *existing_manager,
                                      stack_type *new_manager) {
    uint64_t result = atomicCAS((unsigned long long int *)managers + p2,
                                (unsigned long long int)existing_manager,
                                (unsigned long long int)new_manager);
  ...
  __device__ stack_type *get_manager_atomic(int p2) {
    uint64_t result =
        atomicCAS((unsigned long long int *)managers + p2, 0ULL, 0ULL);

    return (stack_type *)result;
  }

  __device__ bool swap_dead_list_atomic(int p2, stack_type *existing_manager,
                                        stack_type *new_manager) {
    uint64_t result = atomicCAS((unsigned long long int *)dead_list + p2,
                                (unsigned long long int)existing_manager,
                                (unsigned long long int)new_manager);
  ...
  __device__ stack_type *get_dead_list_atomic(int p2) {
    uint64_t result =
        atomicCAS((unsigned long long int *)dead_list + p2, 0ULL, 0ULL);
  ...
  __device__ void *malloc_free_table(uint64_t bytes_requested,
                                     hash_table *cms_table, header *heap) {
#if COUNTING_CYCLES
    ...
    while (true) {
      ...
      while (local_manager != nullptr) {
        ...
        if (malloced == nullptr) {
          ...
          // This lock signifies competition between threads that have traversed
          // + malloced

          // if (!local_manager->request_lock()){
          // 	//the local manager is already being worked on! I can safely
          // leave 	return malloced;
          // }

          // 2) gather necessary variables for swap

          // Do we need to maintain that only one nullptr exists in the list?
          // may be important to create a loop before unwinding this will force
          // threads already traversing to continue until the swap is done I
          // think this is safer so that's what we will do otherwise multiple
          // threads could attempt to add new stacks to the list, which is
          // wasteful and potentially memory-leaky
          ...
          // need to make sure no one else messes with the node structure while
          // we allocate mallocs happen uninterrupted, but no one else can claim
          // the node being moved and new new allocations can occur this should
          // be fine as by construction this node is still serving mallocs.
          // A->stall_lock();
          ...
          // //precondition that the dead list used to point to the head for
          // draining, so if it's not nullptr we are done?
          // //A is now attached to the end of the dead list, and the end of
          // A->B points to C, the new head.
          // //If we do a check to make sure that the dead list is actually
          // real, I think we can return!

          // //New list is C->On, A->B have been disconnected, with B draiining
          // towards A

          // //We can just attempt an atomic swap, if it succeeds we know the
          // list was empty!
          ...
          uint64_t sub_allocator_total_time =
              (sub_allocator_counter_end - sub_allocator_counter_start) /
              COMPRESS_VALUE;

          atomicAdd((unsigned long long int *)&sub_allocator_regular_counter,
                    (unsigned long long int)sub_allocator_total_time);

          atomicAdd((unsigned long long int *)&sub_allocator_main_traversals,
      ...
      // This means that nodes can stall in this loop when other nodes are being
      // shifted only occurs iff
      ...
      if (try_lock_manager(p2_needed)) {
        ...
        if (current_manager != my_manager_head) {
          ...
          // this could be a variation of the ABA problem but I think that will
          // be very rare.
        ...
        // if that condition is met we expect to move the entire stack list into
        // the dead list
        //  and replace it with a new stack.

        // the dead list already points to the head, just need to make sure the
        // free list points to the new node before dropping. All threads moved
        // to the dead list should drain to the free list.

        // I own the lock, append to this manager
        stack_type *new_stack =
            stack_type::init_from_free_list(heap, 1ULL << (2 + p2_needed));
        ...
        if (new_stack != nullptr) {
          ...
          if (current_manager == nullptr) {
            ...
            if (dead_list_head == nullptr) {
              ...
              if (!swap_dead_list_atomic(p2_needed, dead_list_head,
                                         current_manager)) {
          ...
          atomicAdd((unsigned long long int *)&sub_allocator_counter,
                    (unsigned long long int)sub_allocator_total_time);

          atomicAdd((unsigned long long int *)&sub_allocator_alt_traversals,
                    (unsigned long long int)1);
...
//__trap();
      ...
      // stack_type * new_stack = stack_type::init_from_free_list(heap, 1ULL <<
      // (2+i));
    }
  }
  ...
  __device__ void report(reporter *my_reporter) {
    ...
    for (int i = 0; i < maximum_p2 - 2; i++) {
      ...
      // probably don't ever call this on a live allocation but it shouldn't
      // hurt
...
struct log2_template {
  enum { value = 1 + log2_template<x / 2>::value };
};
...
struct dead_list_sub_allocator_wrapper {
  using sub_allocator_type =
      dead_list_sub_allocator<bytes_per_substack,
                              log2_template<max_size - 1>::value + 1>;
};
