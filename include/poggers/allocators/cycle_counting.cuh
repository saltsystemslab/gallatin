...
__host__ void poggers_reset_cycles() { reset_cycles_kernel<<<1, 1>>>(); }
...
__global__ void display_cycles_kernel() {
  ...
  printf("Est. cycles booting stack: %llu, %llu\n", stack_init_counter,
         stack_init_traversals);

  double kernel_avg = ((double)kernel_counter) / kernel_traversals;

  double sub_main_avg =
      ((double)sub_allocator_regular_counter) / sub_allocator_main_traversals;

  double sub_alt_avg =
      ((double)sub_allocator_counter) / sub_allocator_alt_traversals;
...
__host__ void poggers_display_cycles() { display_cycles_kernel<<<1, 1>>>(); }
