...
struct stack_of_stacks_manager {
  static_assert(bytes_per_stack != 0 &&
                (bytes_per_stack & (bytes_per_stack - 1)) == 0);
  // using offset_type = first_matching_address<bytes_per_item>;

  // using bytes_remaining_type = find_offset<bytes_per_stack,
  // offset_type::value>;
  ...
  using my_type = stack_of_stacks_manager<stacks_per_manager, bytes_per_stack,
                                          auto_dealloc>;
  ...
  __device__ static my_type *init_stack_of_stacks(void *ext_address) {
#if DEBUG_PRINTS
    printf(
        "Initializing new stack of stacks with %llu bytes per item, %llu "
        "overall\n",
    ...
    pointer_type *head = pointer_type::init_stack(
        ext_address, (unsigned long)sizeof(my_type), bytes_per_item);
  ...
  __device__ static my_type *init_stack_of_stacks_free_list(
      header *free_list, int bytes_per_item) {
    // request an address such that all stacks that are a part of me are aligned
    void *address_to_use = free_list->malloc_aligned(
        bytes_per_stack * stacks_per_manager + sizeof(my_type), bytes_per_stack,
        -1 * sizeof(my_type));
  ...
  __device__ static void free_stack_of_stacks(header *free_list,
                                              my_type *stack) {
  ...
  __device__ void *malloc_stack() { return stack_head.malloc(); }
