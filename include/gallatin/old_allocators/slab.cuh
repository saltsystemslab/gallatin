...
struct alloc_bitarr {
  ...
  __device__ void attach_allocation(void *ext_alloc) { memmap = ext_alloc; }

  // request one item for this thread
  __device__ bool bit_malloc(void *&allocation, uint64_t &remainder,
                             void *&remainder_offset, bool &is_leader) {
    // group
    //
      ...
      if (active_threads.ballot(ballot_bit_set)) {
        ...
        if (active_threads.thread_rank() == 0) {
          printf("%d/%d sees ext_bits for %d as %llx\n",
                 active_threads.thread_rank(), active_threads.size(),
                 allocation_index_bit, ext_bits);
        }
#endif
        ...
        if (active_threads.thread_rank() + 1 <= ext_bits.get_fill()) {
          ...
          while (true) {
            ...
            if (searching_group.thread_rank() == 0) {
              printf("Leader is %d/%d, sees ext bits as %llx\n",
                     coalesced_threads.thread_rank(), searching_group.size(),
                     ext_bits);
            }
#endif
            ...
            // if the exclusive OR result doesn't contain your bit you are free
            // to modify!

            // last thread knows the true state of the system, so broadcast
            // changes.

            uint64_t my_mask = (1ULL) << my_index;

            // now scan across the masks
            uint64_t scanned_mask = cg::exclusive_scan(searching_group, my_mask,
                                                       cg::bit_or<uint64_t>());
            ...
            ext_bits =
                searching_group.shfl(ext_bits, searching_group.size() - 1);
          ...
          int lowest_fill =
              cg::reduce(coalesced_threads, my_fill, cg::less<int>());

          int leader =
              __ffs(coalesced_threads.ballot(lowest_fill == my_fill)) - 1;
  ...
  __device__ bool bit_free(void *allocation, uint64_t size_in_bytes) {
    ...
    uint64_t scanned_mask =
        cg::inclusive_scan(starting_team, my_mask, cg::bit_or<uint64_t>());

    if (starting_team.thread_rank() == starting_team.size() - 1) {
      if (alloc_bits[upper_bit].set_OR_mask(scanned_mask) |
          scanned_mask == (~0ULL)) {
  ...
  __device__ bool attach_buffer(void *ext_buffer, uint64_t ext_bits) {
    ...
    while (local_copy.get_fill() != 64) {
      ...
      printf("Bit chosen is %d / %llx, %llx\n", allocation_index_bit,
             manager_bits, alloc_bits[allocation_index_bit]);
#endif
    ...
    while (local_copy.get_fill() != 0ULL) {
      ...
      if (active_threads.ballot(ballot_bit_set)) {
        ...
        if (active_threads.thread_rank() + 1 <= ext_bits.get_fill()) {
          ...
          if ((ext_bits.get_fill() > 0) &&
              (leader == coalesced_threads.thread_rank())) {
...
__device__ bool alloc_with_locks(void *&allocation, alloc_bitarr *manager,
                                 storage_bitmap *block_storage) {
  ...
  bool bit_malloc_result =
      manager->bit_malloc(allocation, remainder, remainder_offset, is_leader);
...
struct slab_retreiver {
  ...
  __device__ void *get_mem_ptr() { return memory; }
};
...
struct slab_storage {
  ...
  __device__ void init_random_index(alloc_bitarr *slab) {
    while (true) {
      ...
      if (!(slab_markers.set_control_bit_atomic(index) &
