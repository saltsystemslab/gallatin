...
__global__ void beta_set_block_bitarrs(offset_alloc_bitarr **blocks,
                                       uint64_t num_blocks) {
...
struct per_size_pinned_blocks {
  ...
  static __host__ per_size_pinned_blocks *generate_on_device(
    ...
    host_version->block_bitmap =
        malloc_bitarr::generate_on_device(num_blocks, false);

    host_version->blocks =
        poggers::utils::get_device_version<offset_alloc_bitarr *>(num_blocks);
  ...
  __device__ bool swap_out_block(offset_alloc_bitarr *block) {
    ...
    return (atomicCAS((unsigned long long int *)&blocks[my_smid],
                      ...
                      0ULL) == (unsigned long long int)block);
  }

  __device__ bool replace_block(offset_alloc_bitarr *old_block,
                                offset_alloc_bitarr *new_block) {
    int my_smid = poggers::utils::get_smid() % num_blocks;
  ...
  __device__ bool swap_out_nullptr(offset_alloc_bitarr *block) {
    ...
    return (atomicCAS((unsigned long long int *)&blocks[my_smid], 0ULL,
                      (unsigned long long int)block) == 0ULL);
  }
