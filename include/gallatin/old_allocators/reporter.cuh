...
// CMS reporter
// This object traces over all allocated structure to get a summary of memory in
// use. along with some tools to nicely spit out the results This is generated
// when you call shibboleth->report();
...
struct reporter {
  ...
  __device__ void modify_recording(int modification, int selection) {
    while (true) {
      ...
      if (atomicCAS((unsigned long long int *)&recordings + selection,
  ...
  __device__ uint64_t get_stack_bytes_malloced() { return recordings[0]; }
  __device__ uint64_t get_stack_bytes_free() { return recordings[1]; }

  __device__ uint64_t get_dead_bytes_malloced() { return recordings[2]; }
  __device__ uint64_t get_dead_bytes_free() { return recordings[3]; }

  __device__ uint64_t get_heap_bytes_total() { return recordings[4]; }

  __device__ uint64_t get_heap_bytes_free() { return recordings[5]; }

  __device__ uint64_t get_total_stacks() { return recordings[6]; }

  __device__ uint64_t get_dead_stacks() { return recordings[7]; }

  __device__ uint64_t get_fragmentation() { return recordings[8]; }
};
