...
struct bitarr_grouped {
  ...
  __device__ int determine_my_alloc_relative(uint64_t *&address,
                                             uint64_t &metadata) {
    while (true) {
  ...
  __device__ uint64_t malloc(uint64_t this_id) {
    ...
    while (my_lead_bits) {
      ...
      if (leader % active_threads.size() == active_threads.thread_rank()) {
        ...
        // this is buggy
        old_result =
            atomicExch((unsigned long long int *)(bits + leader), 0ULL);
      ...
      old_result =
          active_threads.shfl(old_result, leader % active_threads.size());
  ...
  // metadata malloc - this is called for all threads and controls some
  // important functionality this needs to return multiple values so the control
  ...
  __device__ cg::coalesced_group metadata_malloc(void *&my_malloc,
                                                 bool &should_preempt,
                                                 uint64_t *&address,
                                                 uint64_t &remaining_metadata) {
    uint64_t my_lead_bits = poggers::utils::ldca(&lead_bits);
      ...
      if (allocations_left > active_threads.thread_rank()) {
        ...
        if (mask_active_group.size() != active_threads.size()) {
          printf("Sync error here! %d, %d != %d\n", allocations_left,
                 mask_active_group.size(), active_threads.size());
        }
        ...
        my_malloc =
            (void *)((uint64_t)ptr_to_memory +
                     bytes_per_item * (determine_my_alloc_relative(
                                           address, remaining_metadata) +
                                       leader));
        ...
        uint64_t new_metadata = cg::reduce(
            mask_active_group, remaining_metadata, cg::less<uint64_t>());
...
struct storage_bitmap {
  ...
  static __host__ my_type *generate_buffers() {
    ...
    init_bitmaps<my_type>
        <<<(maximum_sm - 1) / 512 + 1, 512>>>(bitmaps, maximum_sm);
  ...
  __device__ bool attach_buffer(void *my_buffer, uint64_t my_allocations) {
    ...
    while (my_lead_bits) {
      // SETUP - this isn't working because the other system does not properly
      // drain the main ihput. printf("Stalling %d\n", __popcll(my_lead_bits));

      const auto leader = __ffsll(my_lead_bits) - 1;

      uint64_t old = atomicCAS((unsigned long long int *)(bits + leader), 0ULL,
                               (unsigned long long int)my_allocations);
      ...
      if (old != 0ULL) {
        ...
        atomicExch((unsigned long long int *)(addresses + leader),
                   (unsigned long long int)my_buffer);
  ...
  __device__ void *malloc_from_existing() {
    ...
    while (my_lead_bits) {
      ...
      // atomic swap
      while (old_result) {
        // moved active_threads into the lower partition so that it is refreshed
        // by the loop;
        cg::coalesced_group active_threads = cg::coalesced_threads();
        ...
        if (old_result | (1ULL << (start + active_threads.thread_rank()))) {
          ...
          if ((old_result & mask) == 0ULL &&
              active_threads.thread_rank() == 0) {
            printf("Unsetting %d\n", leader);
            atomicAnd((unsigned long long int *)&lead_bits, ~(1ULL << leader));
          }

          return (void *)(((uint64_t)addresses[leader]) +
                          bytes_per_item *
                              (start + active_threads.thread_rank()));
        }
...
// 		//first, need to determine which threads in my warp are
// participating 		cg::coalesced_group active_threads = coalesced_threads();
...
// 			uint64_t my_address =
// hasher.hash(active_threads.thread_rank()) % 64;

// 			uit64_t my_bits = poggers::utils::ldca(bits+my_address);

// 			//ballot if we think my bits could fulfill all
// allocations 			int largest = cg::reduce(active_threads, __popcll(my_bits),
// cg::greater<int>());

// 			bool ballot = (__popcll(my_bits) == largest ||
// __popcll(my_bits) >= size);
...
// 	       			//if we made it here no successes, decrement
// leader 	       			ballot_result  ^= 1UL << leader;
