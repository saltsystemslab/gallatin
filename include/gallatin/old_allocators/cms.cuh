...
// shibboleth * manager = poggers::allocators::shibboleth::init() or
// init_managed() for host-device unified memory.
//  The amount of memory specified at construction is all that's available to
//  the manager, so you can spin up multiple managers for different tasks or
//  request all available memory! The memory returned is built off of cudaMalloc
//  or cudaMallocManaged, so regular cuda API calls are fine. Unlike the cuda
//  device API, however, you can safely cudaMemcpy to and from memory requested
//  by threads!
...
__global__ void host_malloc_kernel(void **stack_ptr, cms *shibboleth,
                                   uint64_t num_bytes) {
  uint64_t tid = threadIdx.x + blockIdx.x * blockDim.x;
...
template <std::size_t bytes_per_substack, std::size_t num_suballocators,
          std::size_t maximum_p2>
struct shibboleth {
  ...
  using hash_table = poggers::tables::static_table<
      uint64_t, uint16_t,
      poggers::representations::dynamic_container<
          poggers::representations::key_container, uint64_t>::representation,
      1, 4, poggers::insert_schemes::bucket_insert, 20,
  ...
  static __host__ my_type *init_backend(std::size_t bytes_requested,
                                        bool managed) {
    ...
    for (int i = 0; i < num_suballocators; i++) {
      host_cms->allocators[i] =
          host_allocate_sub_allocator<allocator>(host_cms->allocated_memory);
    ...
    my_type *dev_cms =
        (my_type *)host_cms->allocated_memory->host_malloc_aligned(
            sizeof(my_type), 16, 0);
  ...
  __device__ allocator *get_randomish_sub_allocator() {
    ...
    return allocators[randomish_hasher.hash(very_random_number) %
                      num_suballocators];
  }
  ...
  __device__ void *cms_malloc(uint64_t num_bytes) {
    ...
    if (allocator::can_malloc(num_bytes)) {
      void *val_to_ret =
          get_randomish_sub_allocator()->template malloc_free_table<hash_table>(
              num_bytes, free_table, allocated_memory);
      ...
      atomicAdd((unsigned long long int *)&kernel_counter,
                (unsigned long long int)total_time);

      atomicAdd((unsigned long long int *)&kernel_traversals,
  ...
  __device__ void one_thread_report() {
    ...
    printf(
        "Live stacks using %llu bytes of %llu bytes given to substacks "
        "allocated, %f full\n",
        malloced, free, ratio);
    ...
    printf("System Stack utilization: %llu/%llu %f\n", malloced + dead_malloced,
           free + dead_free,
           1.0 * (malloced + dead_malloced) / (free + dead_free));

    printf("Heap using %llu/%llu, %f, fragmentation: %f\n", heap_used,
           heap_total, heap_ratio, fragmentation);

    cms_free_force_heap(my_reporter);
  }

  __host__ void host_report() { host_report_wrapper<my_type>(this); }

  __host__ static void print_info() {
    printf(
        "Allocator with %llu bytes per substack, %llu sub_allocators, size "
        "runs from 4-%llu\n",
