...
struct simple_allocator_one_size {
  ...
  // 				slab_retreiver * next_block =
  // my_bitbuddy->malloc(sizeof(slab_retreiver));

  // 				void * memory_for_blocks =
  // my_bitbuddy->malloc(alloc_size*4096*31);

  // 				if (next_block == nullptr || memory_for_blocks ==
  // nullptr){

  // 					printf("Buddy allocator out of space for
  // allocations!\n"); 					return;
  // 				}
  ...
  __device__ void load_new_slab(int index) {
    alloc_bitarr *slab =
        (alloc_bitarr *)my_bitbuddy->malloc(sizeof(alloc_bitarr));
  ...
  __device__ bool free(void *allocation) { return true; }
  ...
  static __host__ my_type *generate_on_device() {
    ...
    std::cout << "Constructed allocator in " << dev_diff.count()
              << " seconds\n";
