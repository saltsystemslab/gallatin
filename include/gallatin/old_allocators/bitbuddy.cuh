...
__global__ void setup_first_level(uint64_t *items,
                                  uint64_t num_uints_lowest_level,
                                  uint64_t num_items_lowest_level) {
...
// Given a next level, initialize the level above it. This is (AFAIK) fanout
// agnostic.

// setup needs to check how many items there are

__global__ void setup_level(uint64_t *current_level,
                            uint64_t num_items_in_level, bool clean) {
  uint64_t tid = threadIdx.x + blockIdx.x * blockDim.x;
  ...
  if (!clean && tid == (num_items_in_level - 1)) {
    // set to 10
    atomicOr((unsigned long long int *)current_level + my_uint64_t,
             (1ULL << (my_inner * 2)));
...
// uses teams of buddies managed with bitmaps to quickly allocate and deallocate
// large blocks of memory this allocator assigns two bits to every allocation
// type: one bit for has_valid_children and one bit for allocable Together these
// allow for fast traversal of the tree while still maintaining constant-time
// allocs once a suitable match has been found. The process is this: The size at
// the top is known, along with a target While

// TODO: Ask prashant about 3rd bit per item! This could handle allocations of
// scaling size. - Since fanout is 32x, we can build larger-ish allocations by
// grabbing contiguous segments and then mark that those segments are together
// with a unary counter.

// TODO list:
//  1) Init - every item should contribute bits to layer above it, repeat until
//  only one layer is left 2) Malloc - taken from top rec
...
// swap procedure - setting the first bit to 0 means that the state of the
// system is not configurable by other threads
...
//  if it was already 0, we didn't do anything and do not own this node (read
//  failure) if it was 1, we might own the node! check if previous state was 10
...
// if the state was 00 or 01 before we did something wrong and did not really
// allocate this necessitates a rollback of our changes in the lower levels and
// a reset to the original 01 if that was the previous state.

// big change - when allocating a new node, we should originally swap to 01 and
// disable the node then float up this allows for 00 nodes to not exist unless
// the node is explicitly fully alloced this maintains that the top 00 found in
// an items path is the item on free so we start at the top and float down until
// we see a 00.
...
struct bitbuddy_allocator {
  ...
  __host__ __device__ static uint64_t get_num_items_next_level(
      uint64_t level_below) {
    return (level_below - 1) / 32 + 1;
  }

  __host__ __device__ static uint64_t get_size_next_level(
      uint64_t level_below) {
  ...
  static __host__ bitbuddy_allocator *generate_on_device(
      void *ext_memory, uint64_t num_bytes, uint64_t bytes_at_lowest_level) {
    ...
    // With the current scheme, we can pack up to 32 items into one bin
    // simultaneously. uint64_t num_uints_lowest_level = (num_items_lowest_level
    // -1 )/32 + 1;

    // I think setup is identical regardless of fanout? that's pretty neat. May
    // require special control logic
    ...
    while (num_items_current_level > 1) {
      // the number of items in the next level is also the number of uint64s
      // needed to contain the current level!
      uint64_t num_items_next_level =
          get_num_items_next_level(num_items_current_level);

      uint64_t *current_level_bitvector;

      cudaMalloc((void **)&current_level_bitvector,
                 sizeof(uint64_t) * num_items_next_level);

      cudaMemset(current_level_bitvector, 0,
                 sizeof(uint64_t) * num_items_next_level);

      // this call will init the layer.
      setup_level<<<(num_items_current_level - 1 / 512 + 1), 512>>>(
          current_level_bitvector, num_items_current_level, clean);
    ...
    // at the end, num_items current_level = 1, final level is malloced. //num
    // levels is correct, and the bitvectors are initialized. I think this is
    // all correct, will assert in boot tests
    ...
    cudaMemcpy(levels_arr, ext_levels_rev.data(),
               sizeof(uint64_t *) * num_levels, cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(dev_version, host_version, sizeof(bitbuddy_allocator),
  ...
  static __host__ void free_on_device(bitbuddy_allocator *dev_allocator) {
    ...
    cudaMemcpy(&host_alloc, dev_allocator, sizeof(bitbuddy_allocator),
               cudaMemcpyDeviceToHost);

    uint64_t **host_array;

    cudaMallocHost((void **)&host_array,
                   sizeof(uint64_t *) * host_alloc.num_levels);

    cudaMemcpy(host_array, host_alloc.levels,
               sizeof(uint64_t *) * host_alloc.num_levels,
               cudaMemcpyDeviceToHost);
  ...
  __device__ bool check_at_level(int &level, uint64_t bytes_at_level,
                                 uint64_t bytes_needed) {
    // if descended to the lowest level you can't go deeper
    if (level >= (num_levels - 1)) {
      return true;

    } else if (bytes_at_level <= bytes_needed &&
               get_size_next_level(bytes_at_level) > bytes_needed) {
      return true;
    }
  ...
  __device__ bool inline descend(int &level, uint64_t &offset) {
    ...
    if (level >= LEVEL_CUTOF) {
      index =
          shrink_index(levels[level][offset].get_random_active_bit_control());

    } else {
      index =
          shrink_index(levels[level][offset].get_first_active_bit_control());
    }
  ...
  __device__ bool inline descend_to_bit(int &level, uint64_t &offset,
                                        int &index) {
    offset = offset * 32 + index;
    level += 1;
    return true;
  ...
  __device__ int float_up_level(int level, uint64_t &offset,
                                uint64_t &prev_level) {
    ...
    if (prev_level != 0ULL) {
      ...
      prev_level =
          levels[level][internal_offset].unset_control_bit_atomic(index);
  ...
  // 10 or 00 trigger an abort and rollback of the changes, starting from the
  // bottom.
  ...
  inline __device__ void *malloc_from_level(int level, uint64_t offset) {
    // inline __device__ void * malloc_from_level(int & level, uint64_t &
    // offset, int & index){
    ...
    int index =
        shrink_index(levels[level][offset].get_random_active_bit_full());
  ...
  __device__ int select_and_set_random_index(int level, uint64_t offset) {
    while (true) {
      ...
      if (level < LEVEL_CUTOF) {
        index = shrink_index(
            levels[level][offset].get_random_active_bit_control_only());
  ...
  // if there are no routes, ascend up and select a new bit (read old
  // atomically?) when the level we are at is satisfactory for a malloc,
    ...
    while (progress < PROG_CUTOFF) {
      ...
      if (check_at_level(level, bytes_at_level, num_bytes)) {
        ...
        if (!descend(level, offset)) {
          ...
          // some changes must have occured, or a lower level would have been
          // available.
          levels[level][offset].global_load_this();
        }
      }
  ...
  __device__ void correct_up_v2(int level, uint64_t offset, uint64_t items) {
    ...
    do {
      ...
      if ((!items) && (level > 0)) {
        uint64_t next_items =
            levels[level - 1][offset / 32].unset_both_atomic(offset % 32);
  ...
  __device__ void correct_up_v3(int level, uint64_t offset, uint64_t items,
                                int index) {
    // uint64_t items = levels[level][offset].global_load_this();
  ...
  __device__ void float_up_v2(int level, uint64_t offset, uint64_t items) {
    ...
    if ((!(~items))) {
      ...
      if (levels[level][offset].try_swap_empty()) {
        uint64_t next_items =
            levels[level - 1][offset / 32].set_both_atomic(offset % 32) |
            READ_BOTH(offset % 32);
        // set the level above
      ...
      uint64_t next_items =
          levels[level - 1][offset / 32].set_control_bit_atomic(offset % 32) |
          SET_SECOND_BIT(offset % 32);
  ...
  // on free, set bits up, unless all the bits are set!
  __device__ int float_up_v3(int level, uint64_t offset, uint64_t items,
                             int index) {
    ...
    if (__popcll(~(items | READ_BOTH(index))) == 0) {
      ...
      uint64_t next_items =
          levels[level - 1][offset].unset_both_atomic(next_index) &
          (~READ_BOTH(next_index));
  ...
  __device__ void correct_float_up(int corrections, int level,
                                   uint64_t offset) {
    while (corrections > 1) {
  ...
  __device__ void correct_up(int level, uint64_t &offset,
                             uint64_t &current_uint) {
    // couple of cases.
  ...
  __device__ void free_old(void *alloc) {
    ...
    while (true) {
      ...
      if (!(old_val & READ_BOTH(index))) {
        ...
        if (!ascend(level, offset)) {
          printf(
              "Bitbuddy thread %llu ran into a double free at level %d offset "
              "%llu\n",
  ...
  // instead of multiplying by 32 each time, we know that it is 2^5 so a large
  ...
  // In malloc, we are looking for a selection of our size that is 11, and we
  // want to convert it to 00 do this by setting it to 01 first
};
