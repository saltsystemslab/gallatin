...
__global__ void host_malloc_aligned_kernel(free_list_type *heap,
                                           void **writable_address,
                                           uint64_t num_bytes,
                                           uint64_t alignment, int offset) {
...
struct header {
  ...
  // returns true if block has at least size bytes available aligned to
  // alignment - offset
  __device__ bool has_valid_split(uint64_t size, uint64_t alignment,
                                  int offset) {
  ...
  // this includes the header / footer in num_bytes, as is convention for this
  // library not really non locking at the moment [TODO] so the name is probably
  // confusing if you're not me :D
  ...
  __device__ header *try_get_prev_node_locked() {
    ...
    uint64_t alt_size =
        atomicCAS((unsigned long long int *)prev_footer, 0ULL, 0ULL);
  ...
  __device__ void free(void *uncasted_address) { free_safe(uncasted_address); }
  ...
  // 				header * ideal_node =
  // main->split_non_locking(num_bytes);
  ...
  // 				char * ideal_address = ((char *) ideal_node) +
  // 32;
  ...
  // 				printf("Need %llu bytes, just freeing whole
  // node\n");
  ...
  __device__ void *find_first_safe_aligned(uint64_t num_bytes,
                                           uint64_t alignment, int offset) {
    // printf("Starting ")
    stall_lock();
    ...
    while (true) {
      ...
      uint64_t distance_above_alignment =
          (main_as_uint + 32 + offset) % alignment;
      ...
      if (distance_above_alignment == 0) {
        ...
        if (main->get_size() >= num_bytes) {
          ...
          if (leftover > CUTOFF_SIZE && leftover > 48) {
            // case 1
            // we are already aligned! we just need to siphon off extra space
            // somewhere else
            ...
            // case 2
            // there is enough space for this node, but not enough to generate a
            // leftover
      ...
      // distance to next is the distance from the main ptr to the aligned
      // address we we need to subtract 32 bytes
  ...
  __device__ void *malloc_safe(uint64_t bytes_requested) {
    ...
    uint64_t free_list_total_time =
        (heap_counter_end - heap_counter_start) / COMPRESS_VALUE;

    atomicAdd((unsigned long long int *)&free_list_counter,
              (unsigned long long int)free_list_total_time);

    atomicAdd((unsigned long long int *)&heap_traversals,
  ...
  __device__ void *malloc_aligned(uint64_t bytes_requested, uint64_t alignment,
                                  int offset) {
    // bytes of an object contains header + footer
    bytes_requested += 48;
    ...
    void *my_malloc =
        find_first_safe_aligned(bytes_requested, alignment, offset);
  ...
  __host__ void *host_malloc_aligned(uint64_t bytes_requested,
                                     uint64_t alignment, int offset) {
    ...
    host_malloc_aligned_kernel<header>
        <<<1, 1>>>(this, temp_write_space, bytes_requested, alignment, offset);
  ...
  __device__ void printnode() {
    ...
    printf(
        ...
        "is_locked: %d, is alloced: %d, footer %llu\n",
        tid, this, lock_and_size.as_bitfield.size, get_next(), get_prev(),
        check_endpoint(), check_lock(), check_alloc(), get_footer()[0]);
  }
...
__device__ inline atomicLock::~atomicLock() { header_to_lock->unlock(); }
