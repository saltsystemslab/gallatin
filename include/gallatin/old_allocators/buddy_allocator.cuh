...
//  so cas needs to take EXT value for consistency - we need to use c.next on
//  remove instead of swapping away whatever is in A.next
...
//  only items which have been properly marked as deleted can be resinserted
//  into the list
...
// Can quickly pointer swing without looking at main memory - this is a big plus
// to it not being in place Singly linked.
...
struct buddy_node {
  ...
  __device__ void init() { pointer_counter = 0ULL; }

  __device__ uint64_t inline generate_alloc_mask() { return 1ULL; }

  __device__ uint64_t inline generate_unset_alloc_mask() {
    return ~generate_alloc_mask();
  }

  __device__ uint64_t inline generate_delete_mask() { return 2ULL; }

  __device__ uint64_t inline generate_unset_delete_mask() {
    return ~generate_delete_mask();
  }

  // we know the buddy allocator is aligned to large blocks, so the first 4 bits
  // are always available for a counter.
  ...
  __device__ uint64_t get_value() { return pointer_counter; }
  ...
  __device__ uint64_t try_attach_new(uint64_t expected, uint64_t new_pointer) {
    ...
    (atomicCAS((unsigned long long int *)&pointer_counter,
               (unsigned long long int)expected,
               (unsigned long long int)(new_pointer | next_counter)));
  }

  __device__ uint64_t get_my_ptr_as_uint() { return (uint64_t)this; }
};

// setup occurs in parallel for speed since we could potentially scale far out
// of GPU mem this has to be templated because cuda doesn't let objects have
// __global__ methods so this is called from a dummy host method that sets it up
...
__global__ void init_memory_pointers(memory_allocator *mem) {
  ...
  for (int i = 0; i < num_levels; i++) {
    if (tid < (items_at_top + prev_passed)) {
      ...
      // if we maintain these as relative offsets may be able to get away with
      // uints? - no need to be ptrs as we can step backwards.
...
struct buddy_allocator {
  ...
  __device__ uint64_t get_num_items_top() { return num_items_top_level; }

  __device__ int get_max_p2() { return max_p2; }

  __device__ int get_min_p2() { return min_p2; }
  ...
  static __host__ my_type *generate_on_device(void *ext_memory,
                                              uint64_t num_bytes) {
    // Assert alignment
    ...
    init_memory_pointers<<<(num_items_to_allocate - 1) / 1024 + 1, 1024>>>(
        dev_version);
