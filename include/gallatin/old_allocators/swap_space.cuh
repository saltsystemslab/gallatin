...
// The goal of this code is to abstract the transition from host/device code for
// CUDA
//  data structures, such that uses can write complex data structs and have them
//  scale automatically to disk.
// As per the original implementation, we define a swap_space::pointer type that
// acts a device pointer.
...
struct swap_space {
  ...
  __host__ my_type *generate_on_device(dev_allocator *dev_alloc,
  ...
  __host__ void free_on_device(my_type *dev_version) { cudaFree(dev_version); }
  // convert a raw cuda pointer to a new host pointer, and return the host
  // pointer
  template <typename object_type>
  __device__ void serialize(object_type *dev_pointer) {
    object_type *host_ptr =
        (object_type *)host_memory->malloc(sizeof(object_type));
  ...
  __device__ void deserialize(object_type *host_ptr) {
    object_type *dev_pointer =
        (object_type *)dev_memory->malloc(sizeof(object_type));
