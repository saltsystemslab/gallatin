...
namespace data_structs {
	...
	template <typename T, T format_code, int level>
	struct formattable_atomic_recursive {

		using my_type = formattable_atomic_recursive<T, format_code, level>;

		T * data;

		using flush_type = formattable_atomic_recursive<uint64_t, ~0ULL, level-1>; 

		flush_type needs_flushed;
		flush_type is_flushed;
		//how to perform swap
		//if resizing, 
		//perform global reads until new keys, vals are available.

		__device__ inline formattable_atomic_recursive(uint64_t nitems): needs_flushed((nitems-1)/64+1), is_flushed((nitems-1)/64+1){
			...
			//needs_flushed = flush_type::get_pointer(total_flush_uint64_t*sizeof(uint64_t));
			//is_flushed = flush_type::get_pointer(total_flush_uint64_t*sizeof(uint64_t));

			//nullptr issues propogate up to get_pointer, need only check data.
			if (data == nullptr){
		...
		__device__ inline bool check_if_need_calloc(uint64_t index){
			...
			//saves atomic on first round.
    		if (needs_flushed.wrappedAtomicAnd(high, ~SET_BIT_MASK(low)) & SET_BIT_MASK(low)){
		...
		__device__ inline T wrappedAtomicExch(uint64_t index, T exchange){
		...
		__device__ inline T wrappedAtomicAnd(uint64_t index, T bits){
		...
		__device__ inline T wrappedAtomicOr(uint64_t index, T bits){
		...
		__device__ inline T wrappedAtomicAdd(uint64_t index, T value_to_add){
		...
		__device__ inline T wrappedAtomicCAS(uint64_t index, T expected, T replace){
		...
		//move constructor
		__device__ inline formattable_atomic_recursive(formattable_atomic_recursive&& other){
		...
		__device__ inline formattable_atomic_recursive operator=(const formattable_atomic_recursive & first){
	...
	//
	template <typename T, T format_code>
	struct formattable_atomic_recursive<T, format_code, 0> {

		using my_type = formattable_atomic_recursive<T, format_code, 0>;
		...
		__device__ inline formattable_atomic_recursive(uint64_t nitems){
		...
		__device__ inline void mark_index_calloced(uint64_t index){
			...
			//atomicAdd((unsigned long long int *)&is_flushed[high], 0ULL);
			if(!(atomicAnd((unsigned long long int *)&is_flushed[high], (unsigned long long int) ~SET_BIT_MASK(low) ) & SET_BIT_MASK(low))){
				//this is bizarre behavior, implies that the region was previously calloced.
