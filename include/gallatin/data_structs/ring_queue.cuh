...
namespace data_structs {
	...
	struct ring_queue {
		...
		static __host__ void free_on_device(my_type * dev_queue){
			...
			cudaFree(host_version->ext_buffer);
		...
		__device__ bool enqueue(T new_item){
			...
			while (true){
				...
				uint64_t slot_active_count = atomicAdd(&active_count, 1U);
		...
		__device__ bool dequeue(T & return_val){
			...
			while (true){
				...
				uint64_t slot_active_count = atomicSub(&active_count, 1U);
				...
				uint64_t dequeue_slot = atomicAdd(&dequeue_counter, 1ULL);
				

				T returnVal = atomicExch(&buffer[default_slot], default_value);

				while (returnVal == default_value){
					returnVal = atomicExch(&buffer[default_slot], default_value);
				}
