...
#include <poggers/counter_blocks/shared_block_slab_allocator.cuh>
...
__global__ void malloc_tests(shared_slab_allocator<15> * allocator, uint64_t max_mallocs){
...
__host__ void boot_slab_one_size(){
   ...
   shared_slab_allocator<15> * test_alloc = shared_slab_allocator<15>::generate_on_device(64000000, 16);
   ...
   shared_slab_allocator<15>::free_on_device(test_alloc);
...
__global__ void allocate_into_array(shared_slab_allocator<num_blocks> * allocator, uint64_t * array, uint64_t num_mallocs, uint64_t * misses){
...
template <int num_blocks>
__global__ void allocate_into_array_bits(shared_slab_allocator<num_blocks> * allocator, uint64_t * array, uint64_t num_mallocs, uint64_t * misses){
...
template <int num_blocks>
__global__ void free_from_array(shared_slab_allocator<num_blocks> * allocator, uint64_t * array, uint64_t num_mallocs){
...
template <int num_blocks>
__global__ void free_from_array_bits(shared_slab_allocator<num_blocks> * allocator, uint64_t * array, uint64_t max_offset_bit, uint64_t * misses){
...
template <int num_blocks>
__global__ void log_free_kernel(shared_slab_allocator<num_blocks> * allocator, uint64_t * array, uint64_t max_offset_bit){
...
__host__ void test_num_malloc_frees_bitarr(uint64_t num_mallocs, int num_rounds){
   ...
   shared_slab_allocator<blocks> * test_alloc = shared_slab_allocator<blocks>::generate_on_device(total_num_allocs, 1);
   ...
   shared_slab_allocator<blocks>::free_on_device(test_alloc);
...
__host__ void test_num_malloc_frees(uint64_t num_mallocs, int num_rounds){
   ...
   shared_slab_allocator<num_blocks> * test_alloc = shared_slab_allocator<num_blocks>::generate_on_device(15000000+num_mallocs, 1);
   ...
   shared_slab_allocator<num_blocks>::free_on_device(test_alloc);
...
__host__ void test_num_malloc_no_free(uint64_t num_mallocs, int num_rounds){
   ...
   for (int i=0; i< num_rounds; i++){
      ...
      shared_slab_allocator<15> * test_alloc = shared_slab_allocator<15>::generate_on_device(15000000+num_mallocs, 1);
...
template <int num_blocks>
__global__ void churn_kernel(shared_slab_allocator<num_blocks> * allocator, uint64_t * bitarray, uint64_t num_threads, uint64_t num_rounds, uint64_t * misses){
...
__host__ void test_churn(uint64_t num_threads, uint64_t allocs_per_thread){
   ...
   shared_slab_allocator<num_blocks> * test_alloc = shared_slab_allocator<num_blocks>::generate_on_device(num_mallocs, 1);
...
int main(int argc, char** argv) {
   ...
   //test_num_malloc_frees_bitarr<108>(num_threads, num_allocs);

   test_churn<108>(num_threads, num_allocs);
