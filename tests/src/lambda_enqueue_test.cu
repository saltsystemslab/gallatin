...
//This test is going to do some tests with adding and removing lambda function pointers from a queue.
//is it fast? IDK.

#include <gallatin/data_structs/ring_queue.cuh>
#include <gallatin/data_structs/functor.cuh>
...
__device__ void print_tid(uint64_t tid){
...
__global__ void enqueue_test_kernel(queue * dev_queue, uint64_t nitems){
   ...
   functor<print_tid, uint64_t> my_functor(tid);

   my_functor.apply_args();
...
__host__ void queue_test(uint64_t n_threads){
   ...
   using queue_type = ring_queue< void (*)() , nullptr>;
