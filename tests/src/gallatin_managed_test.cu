...
__host__ void gallatin_test_allocs_pointer(uint64_t num_bytes, int num_rounds, uint64_t size){
   ...
   gallatin_type * allocator = gallatin_type::generate_on_device_managed(num_bytes, 42);
   ...
   if (num_segments > 0){
      ...
      printf("Gallatin using Managed memory for storage array\n");

      cudaMallocManaged((void **)&bits, sizeof(uint64_t *)*clipped_num_allocs);
