...
__host__ void calloc_test(uint64_t nitems, int num_rounds){
   ...
   using callocable_type = formattable_atomic_recursive<T, 0ULL, stride>;
...
template <typename T, int stride>
__host__ void formattable_recursive_battery(){

   printf("Stride %d\n", stride);

   calloc_test<T, stride>(10000, 10);

   cudaDeviceSynchronize();

   calloc_test<T, stride>(100000, 10);

   cudaDeviceSynchronize();

   calloc_test<T, stride>(1000000, 10);

   cudaDeviceSynchronize();

   calloc_test<T, stride>(10000000, 10);

   cudaDeviceSynchronize();

   calloc_test<T, stride>(100000000, 10);
...
__host__ void array_battery(){
   ...
   array_test<T>(10000, 10);

   cudaDeviceSynchronize();

   array_test<T>(100000, 10);

   cudaDeviceSynchronize();

   array_test<T>(1000000, 10);

   cudaDeviceSynchronize();

   array_test<T>(10000000, 10);

   cudaDeviceSynchronize();

   array_test<T>(100000000, 10);
...
int main(int argc, char** argv) {
   ...
   formattable_recursive_battery<uint64_t,0>();
   formattable_recursive_battery<uint64_t,1>();
   formattable_recursive_battery<uint64_t,2>();
   formattable_recursive_battery<uint64_t,3>();
