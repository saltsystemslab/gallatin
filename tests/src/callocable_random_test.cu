...
__global__ void calloc_query_kernel(callocable_type * callocable_wrapper, uint64_t nitems){
   ...
   uint64_t hash = gallatin::hashers::MurmurHash64A(&tid, sizeof(uint64_t), tid) % nitems;

   //auto my_item = callocable_wrapper[0][hash];

   //if (my_item != 0){

      auto my_item = atomicExch((unsigned long long int *)&callocable_wrapper[0][hash], 0ULL);

      if (my_item != 0){
         printf("Failed to unset address %lu even with atomic check.\n", hash);
         callocable_wrapper[0].print_calloc_status(hash);
      }
...
__global__ void query_array_kernel(T * array, uint64_t nitems){
   ...
   auto my_item = atomicExch((unsigned long long int *)&array[hash], 0ULL);

   if (my_item != 0){
      printf("Array failed to unset address %lu even with atomic check.\n", hash);
      //allocable_wrapper[0].print_calloc_status(hash);
   }
