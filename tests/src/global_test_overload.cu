...
__host__ void gallatin_test_allocs_pointer(uint64_t num_bytes, int num_rounds, uint64_t size){
   ...
   for (int i = 0; i < num_rounds; i++){
      ...
      insert_one_size<<<(num_allocs-1)/TEST_BLOCK_SIZE+1,TEST_BLOCK_SIZE>>>(1.1*num_allocs, size, bits, misses);
      ...
      free_one_size_pointer<<<(num_allocs-1)/TEST_BLOCK_SIZE+1,TEST_BLOCK_SIZE>>>(1.1*num_allocs, size, bits);
      free_timing.sync_end();

      kernel_timing.print_throughput("Malloced", 1.1*num_allocs);

      free_timing.print_throughput("Freed", 1.1*num_allocs);
