...
__global__ void insert_one_size(uint64_t num_inserts, uint64_t size, uint64_t ** bitarray, uint64_t * misses){
   ...
   uint64_t * malloc = (uint64_t *) global_host_malloc(size);
...
__host__ void gallatin_test_allocs_pointer(uint64_t num_bytes, int num_rounds, uint64_t size){
   ...
   init_global_host_allocator(num_bytes, 42);
   ...
   print_global_host_stats();

   cudaFree(misses);

   cudaFree(bits);

   free_global_host_allocator();
